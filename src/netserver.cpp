...
void NetServer::HandleAccept() {
    ...
    acp.async_accept(handler->GetSock(),
                     [this, handler](const boost::system::error_code &ec) {
                         if (ec) {
                             cout << ec.value() << " msg:" << ec.message()
                                  << endl;
                         }
                         //保存连接对象
...
void RWHandle::HandleRead() {
    asio::async_read_until(
        ...
        [this](const boost::system::error_code &ec,
            ...
            std::string data(buffers_begin(_buffer.data()),
                             buffers_begin(_buffer.data()) + _buffer.size());
            cout << "trans :" << bytes_transferred << endl;
            cout << data << endl;
            // boost::asio::async_read_until will call _buffer.commit so not use
            // commit here
...
void RWHandle::eventHandle(std::string_view data) {
    ...
    //确定收到的格式长度是否是希望的大小。
    cout << "packege's length is" << str_list.size() << endl;
    if (str_list.size() != 3) {
        cout << "packege length is not true" << endl;
        return;
    }
    //处理操作
    if (str_list[0] == GET) {
